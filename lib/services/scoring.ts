...
export function buildPackages(input: SearchInput, flights: FlightOption[], hotels: HotelOption[], transfers: TransferOption[]): TravelPackage[] {
  ...
  const cheapest = [...combinations].sort((a, b) => a.cost.finalTotal - b.cost.finalTotal)[0];
  const fastest = [...combinations].sort((a, b) => (a.flight.durationMinutes + a.transfer.durationMinutes) - (b.flight.durationMinutes + b.transfer.durationMinutes))[0];
  const premium = [...combinations].sort((a, b) => (b.flight.comfortScore + b.hotel.rating * 10 + b.transfer.comfortScore) - (a.flight.comfortScore + a.hotel.rating * 10 + a.transfer.comfortScore))[0];
  const family = [...combinations.filter((c) => c.hotel.familyFriendly && c.flight.baggageIncluded)].sort((a, b) => b.hotel.rating - a.hotel.rating)[0] ?? premium;
  const best = [...combinations].sort((a, b) => scorePackage(input, b.flight, b.hotel, b.transfer, minCost, maxDuration) - scorePackage(input, a.flight, a.hotel, a.transfer, minCost, maxDuration))[0];
